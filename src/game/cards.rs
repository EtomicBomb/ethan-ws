...
impl Cards {
    pub const ENTIRE_DECK: Cards = Cards { bits: 0xfffffffffffff };
    pub const SUITS: [Cards; 4] = [Cards::CLUBS, Cards::SPADES, Cards::HEARTS, Cards::DIAMONDS];
    const CLUBS: Cards = Cards { bits: 0x1111111111111 };
    const SPADES: Cards = Cards { bits: 0x2222222222222 };
    const HEARTS: Cards = Cards { bits: 0x4444444444444 }; 
    const DIAMONDS: Cards = Cards { bits: 0x8888888888888 }; 

    pub const fn single(card: Card) -> Cards {
        Cards { bits: 1 << card.index }
    }

    pub fn insert(&mut self, card: Card) {
        self.insert_all(Cards::single(card))
    }
    ...
    pub fn remove(&mut self, card: Card) {
        self.remove_all(Cards::single(card));
    }
    ...
    pub fn contains(self, card: Card) -> bool {
        Cards::single(card).is_subset(self)
    }

    pub fn is_subset(self, other: Cards) -> bool {
        !self.bits | other.bits != 0
    }

    pub fn disjoint(self, other: Cards) -> bool {
        self.bits & other.bits == 0 
    }
    ...
    pub fn all_same_suit(self) -> bool {
        Cards::SUITS.into_iter().any(|s| self.is_subset(s))
    }
...
impl Card {
    pub const THREE_OF_CLUBS: Card = Card { index: 0 };
...
mod tests {
    ...
    fn cards_set_len() {
        ...
        let cards_set = HashSet::from_iter(Cards::ENTIRE_DECK);
    ...
    #[test]
    fn encoding() {
        let s = format!("{}", Card::THREE_OF_CLUBS);
        assert_eq!(s, "3♣");
        let s = serde_json::to_string(&Card::THREE_OF_CLUBS).unwrap();
        assert_eq!(s, "3♣");
    }
